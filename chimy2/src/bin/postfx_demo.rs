...
use chimy2::postfx::{BloomPass, DofPass, FxaaPass, PostChain, SsaoPass, VignettePass};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dof_enabled = args.dof;
    let cycle = !args.bloom && !args.fxaa && !args.vignette && !ssao_enabled && !dof_enabled;
    ...
    run_demo(
        ...
        move |framebuffer: &mut Framebuffer, elapsed: f32, _: &InputState| {
            ...
            let camera = if ssao_enabled {
                ...
                OrbitController::new(Vec3::new(0.0, -0.45, 0.0), 5.4, 0.0, 0.18)
                    .camera(1.0, aspect, 0.1, 100.0)
            } else {
            ...
            let dof_near = lighting_uniform(
                camera,
                Mat4::translate(Vec3::new(-1.6, -0.43, 0.8)),
                Vec3::new(0.08, 0.035, 0.025),
                Vec3::new(0.85, 0.18, 0.07),
                Vec3::new(1.0, 0.8, 0.55),
                32.0,
            );
            let dof_middle = lighting_uniform(
                camera,
                Mat4::translate(Vec3::new(0.0, -0.43, 0.0)),
                Vec3::new(0.06, 0.07, 0.10),
                Vec3::new(0.95, 0.42, 0.16),
                Vec3::new(1.0, 0.85, 0.65),
                32.0,
            );
            let dof_far = lighting_uniform(
                camera,
                Mat4::translate(Vec3::new(1.6, -0.43, -0.9)),
                Vec3::new(0.025, 0.05, 0.08),
                Vec3::new(0.08, 0.34, 0.82),
                Vec3::new(0.55, 0.75, 1.0),
                32.0,
            ...
            if ssao_enabled || dof_enabled {
                let mut chain = PostChain::new();
                if ssao_enabled {
                    chain.push(SsaoPass::new(camera.projection_matrix()));
                }
                if dof_enabled {
                    let mut dof = DofPass::new(camera.projection_matrix());
                    dof.set_focus_distance(4.8);
                    dof.set_aperture(60.0);
                    dof.set_max_coc_radius(7.0);
                    chain.push(dof);
                }
            ...
            pipeline.render(framebuffer, |frame, target| {
                if ssao_enabled || dof_enabled {
                    if dof_enabled {
                        ...
                        frame.draw_mesh(target, &cube, &dof_near);
                        frame.draw_mesh(target, &cube, &dof_middle);
                        frame.draw_mesh(target, &cube, &dof_far);
                    } else {
                        frame.draw_mesh(target, &ground, &ssao_uniforms.0);
