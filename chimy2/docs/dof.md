...
the pass uses a deterministic 20-tap disc gather. it clamps taps at the frame
...
the gather is expensive: each defocused pixel can load up to 20 color samples
and 20 reconstructed depth samples. compare it with and without `--dof` at
