...
| **Box**      | Impl    | Deferred| Deferred| Deferred| Deferred | Deferred  | CCD*    | Impl   |
...
| **Mesh**     | Impl    | Impl    | CCD     | Deferred| Deferred | Deferred  | CCD     | —      |
| **Hfield**   | —       | Impl    | Impl    | Impl    | —        | —         | —       | —      |

`CCD*` marks box-mesh. Its contact count, position, and depth are covered by
the fixture below. Its normal construction remains an open finding with the
measured bound shown in the route table.
...
| mesh-plane | `contact::mesh_plane` | ≤ 2 (plane-convex support points) |
...
sphere-ellipsoid, sphere-mesh, box-mesh, and mesh-mesh. Ellipsoid-plane and
mesh-plane use the support midpoint and two-point manifold rules from
`mjc_PlaneConvex`.
...
`tools/capture_contact_route_probes.py` from the six listed `source_xml`
files. MuJoCo positions are midpoint positions. Its frame normal is shown
after conversion to newt's B-into-A convention. Newt values are from the
...
| P5 | box-mesh | 1 contact; maximum position / normal / depth error `2.5e-7 / 1.0 / 6.0e-8` | fixture bounds `1e-6 / 1.01 / 1e-6` | open normal construction; contact count, position, and depth aligned |
| P6 | mesh-mesh | 1 contact; maximum position / normal / depth error `1.8e-7 / 2.5e-6 / 9.7e-8` | fixture bounds `1e-6 / 1e-5 / 1e-6` | CCD-aligned |

These are measured route-alignment results from the committed fixture. The
fixture keeps four poses per pair and fails if its source XML or mesh data is
missing.

The expanded fixture covers no-contact, shallow, deep, and rotated off-axis
poses for every pair. The route test reads the MuJoCo rows and asserts these
per-pair bounds:

| probe | observed max position / normal / depth error | asserted bounds | count |
|---|---|---|---|
| P1 sphere-ellipsoid | `6.81e-7 / 4.93e-6 / 1.49e-8` | `1e-4 / 1e-4 / 1e-4` | equal |
| P2 sphere-mesh | `2.36e-7 / 2.35e-6 / 2.98e-8` | `1e-4 / 1e-4 / 1e-4` | equal |
| P3 plane-ellipsoid | `1.12e-8 / 0 / 2.24e-8` | `1e-4 / 1e-4 / 1e-4` | equal |
| P4 plane-mesh | `4.1e-8 / 0 / 5.96e-8` | `1e-4 / 1e-4 / 1e-4` | equal |
| P5 box-mesh | `2.5e-7 / 1.0 / 6.0e-8` | `1e-6 / 1.01 / 1e-6` | equal |
| P6 mesh-mesh | `1.8e-7 / 2.5e-6 / 9.7e-8` | `1e-6 / 1e-5 / 1e-6` | equal |
...
NEWT-32 ships only the four aligned route pairs plus box-mesh and mesh-mesh.
...
The enabled box-mesh and mesh-mesh routes have fixture rows in
`contact_route_probes.json`. Their dynamic anchors use executable XML sources
and compare steps 20 and 100. The measured position / orientation / contact
count bounds are `0.9 / 1.1 / 3` and `0.4 / 1.1 / 3` for box-mesh early/full,
and `0.1 / 0.1 / 1` and `0.3 / 0.3 / 1` for mesh-mesh early/full.
...
| `tests/contacts_geoms_v1.rs::analytic_convex_route_probes_are_fixture_backed` | six MuJoCo route probes with executable XML provenance; corrupted fixture data or missing XML fails the test. |
| `tests/contacts_geoms_v1.rs::dynamic_enabled_convex_anchors_are_fixture_backed` | box-mesh drop and mesh-mesh tumble compare fixture-backed early step 20 and full step 100 position, orientation, and contact-count bounds. |
| `tests/contacts_geoms_v1.rs::enabled_convex_ccd_routes_emit_one_contact` | one-contact smoke coverage for the enabled box-mesh and mesh-mesh CCD pairs only. |
...
| CCD simplex termination loosening | `contact::tests::gjk_tetrahedron_enclosure_kills_simplex_termination_mutant` and the fixture-backed box-mesh / mesh-mesh rows |
...
| enabled box-mesh or mesh-mesh pair removed from the support matrix | `world_validate_supported_pairs_rejects_deferred_ccd_pairs` and the fixture-backed route rows |
