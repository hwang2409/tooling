...
| mesh-mesh `CCD` pair | `contact::ccd_convex_contacts` (GJK + EPA + native-style multiCCD) | ≤ 4 |
...
helpers matching MuJoCo's `mjc_Convex` route. Mesh-mesh uses deterministic GJK
plus EPA, then MuJoCo-style multiCCD feature recovery and clipping. It emits up
to four midpoint contacts for face-face and edge-face features. Positive
margins retain the single EPA contact, matching MuJoCo's native path. The
stable geom-id ordering remains unchanged. Ellipsoid-plane and mesh-plane use
the support midpoint and two-point manifold rules from `mjc_PlaneConvex`.
Box-mesh remains deferred because its EPA witness normal is not aligned.

The seven probes use isolated MuJoCo 3.11.0 models with one selected dynamic
...
| P7 | mesh-mesh-manifold | face-face 4, edge-face 2, vertex-face 2; position / normal / depth bounds `4.5e-3 / 1e-6 / 1e-4` | reviewed bounds `5e-3 / 1e-6 / 1e-4` | multiCCD-aligned |
...
NEWT-32 ships the four aligned route pairs plus mesh-mesh. NEWT-37 adds the
mesh-mesh multiCCD manifold and its MuJoCo 3.11.0 fixture-backed P7 probes.
...
| `tests/contacts_geoms_v1.rs::analytic_convex_route_probes_are_fixture_backed` | seven MuJoCo route probes with parsed XML provenance and reviewed bounds; corrupted fixture data or XML fails the test. |
...
| `tests/contacts_geoms_v1.rs::enabled_convex_ccd_routes_emit_one_contact` | one-contact smoke coverage for positive-margin and non-manifold enabled mesh-mesh CCD routes. |
| `tests/contacts_geoms_v1.rs::mesh_mesh_stack_keeps_face_manifold_under_pgs` | three stacked mesh cubes retain four face contacts at the initial interface and remain bounded under PGS. |
