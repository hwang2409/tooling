...
| `<actuator>` | `<position>`, `<velocity>`, `<motor>`, `<general>`, and `<muscle>` children. |
...
| `<general>`  | `name`, `joint` or `tendon`, `gaintype`, `gainprm`, `biastype`, `biasprm`, `gear`, `dyntype`, `dynprm`, `lengthrange`, `ctrlrange`, `forcerange`, `class`, `ctrllimited`, `forcelimited` | Full actuator model. Muscle types require all three type fields to be `muscle` and require explicit `lengthrange`. See [`docs/actuators.md`](actuators.md). |
| `<muscle>`   | `name`, `joint` or `tendon`, `lengthrange`, `range`, `force`, `scale`, `lmin`, `lmax`, `vmax`, `fpmax`, `fvmax`, `timeconst`, `tausmooth`, `gear`, `ctrlrange`, `forcerange`, `class` | MuJoCo muscle shortcut with default curve parameters and `ctrlrange="0 1"`. The loader rejects missing `lengthrange`. |

`<cylinder>`, `<damper>`, and `<intvelocity>` remain outside the subset.
...
1. `biped_simple_pure_pd_smoke` — pure joint PD, **NO external
   ...
   between steps 700 and 900 down to ≤ 20 % of initial height. The
...
step  780  root=(see test output) z_ratio<0.5
...
first crossed z_ratio < 0.5 at step 780
```
