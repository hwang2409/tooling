...
| ------------ | ------------------------------------------------------------------------- | ----- |
| `<position>` | `name`, `joint`, `kp`, `kv` OR `dampratio`, `forcerange`, `ctrlrange`, `class`, `ctrllimited`, `forcelimited`, `target` | `forcerange="lo hi"` maps to a symmetric `clamp = min(|lo|, |hi|)`. `ctrlrange` is accepted but not enforced (use `<general>` for a real ctrl clamp). `dampratio` uses a unit reflected-inertia estimate (`kv = 2·dr·sqrt(kp·1)`); pass `kv` for exact control. |
