...
npm ci --prefix web                                  # pass; 215 packages
npm run lint --prefix web                            # pass
npm run typecheck --prefix web                       # pass
npm test --prefix web -- --run                       # pass, 5 tests
npm run build --prefix web                           # pass; Vite production bundle

python3 -m json.tool contracts/protocol-v1.schema.json >/dev/null  # pass
```

The Python and TypeScript suites both parse the same
`contracts/fixtures/stream.json`. They cover ordered duplicate headers,
missing/empty/truncated body states, response-before-request-end ordering,
decimal-string values above JavaScript's safe integer range, unknown additive
fields/types, redaction canaries, and the private-mitmweb import guard.
